...
export async function buildCertificatePdf(data: any, baseUrl?: string) {
  ...
  const qr = await QRCode.toDataURL(`${origin}/validar/${encodeURIComponent(data.codigo)}`, { margin: 2, width: 200, color: { dark: "#000000", light: "#ffffff" } });
